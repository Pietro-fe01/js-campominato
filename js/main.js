...
window.onload = function(){
    ...
    function bombsGenerator(select){
        let bombsArray = [];
        while(bombsArray.length < 16){
            let numGen = getRndInteger(1, checkSelectValue(select));
    ...
    playGameButton.addEventListener("click", function(){
        if(selectBody.value !== "default"){
            ...
            let bombsList = (bombsGenerator(selectBody));
    ...
    endGameButton.addEventListener("click", function(){
        endGameBlock.classList.add("d-none");
        endGameBlock.classList.remove("d-flex");
        GameScore.classList.add("d-none");
        cellContainer.innerHTML = `<button id="info-button">Choose the difficulty and start to play!</button>`;
    });
}
